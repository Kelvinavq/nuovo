export const Translation = {
  es: {
    title: "Retirar",
    label1d1: "Seleccione el medio de pago",

    option1: "Seleccionar método de retiro",
    option2: "Transferencia entre usuarios",
    option3: "Transferencia externa",
    option4: "Transferencia nacional (Arg)",
    option5: "Efectivo",

    text1:
      "Debe verificar su cuenta antes de realizar un retiro. Puede encontrar información sobre cómo hacerlo en su perfil.",

    button1: "Enviar",

    // transferencia_entre_usuarios

    label1: "Correo electrónico destino",
    label2: "Ingrese el monto a retirar",

    // transferencia_nacional

    label3: "Seleccione la región",
    option6: "Selecciona una región",
    option7: "USA",
    option8: "Europa",

    label4: "Nombre del banco",
    label5: "Routing Number (ACH)",
    label6: "Routing Number (WIRE)",
    label7: "Dirección del banco",
    label8: "Número de cuenta",
    label9: "Nombre del beneficiario",
    label10: "Ingrese el monto a retirar",

    label11: "Nombre del banco",
    label12: "Código SWIFT / BIC",
    label13: "Dirección del banco",
    label14: "Numero de cuenta",
    label15: "Código SORT",
    label16: "IBAN",
    label17: "Ingrese el monto a retirar",

    // transferencia_nacional

    label18: "Selecciona el tipo de pago",
    option9: "Selecciona un método",
    option10: "Banco Virtual",
    option11: "Banco Físico",

    label18: "Ingrese su alias o Número de cuenta nacional (CVU)",
    label19: "Ingrese el CUIT/CUIL",
    label20: "Ingrese el nombre de la cuenta",
    label21: "Ingrese el monto a retirar",

    label22: "Ingrese su alias o Número de cuenta nacional (CBU)",
    label23: " Ingrese el número de caja de ahorros ó cuenta corriente",
    label24: "Ingrese el CUIT/CUIL",
    label25: "Ingrese el nombre de la cuenta",
    label26: "Ingrese el monto a retirar",

    label27: "Ingrese el monto a retirar",



    // swal
    messageEmpty: "Por favor, complete todos los campos obligatorios.",

    swalTitle: "¿Estás seguro de esta transacción?",
    swalText: "Se enviará una solicitud de retiro por el monto:",
    swalButton1: "Sí, estoy seguro",
    swalButton2: "Cancelar",
    
    swalText2: "al usuario:",


  },
  en: {
    title: "Withdraw money",

    label1d1: "Select the means of payment",

    option1: "Select withdrawal method",
    option2: "Transfer between users",
    option3: "External Transfer",
    option4: "National transfer (Arg)",
    option5: "Cash",

    text1:
      "You must verify your account before making a withdrawal. You can find information on how to do this in your profile.",

    button1: "Send",

    // transferencia_entre_usuarios

    label1: "E-mail destination",
    label2: "Enter the amount to withdraw",

    // transferencia_nacional

    label3: "Select the region",
    option6: "Select a region",
    option7: "USA",
    option8: "Europe",

    label4: "Bank name",
    label5: "Routing Number (ACH)",
    label6: "Routing Number (WIRE)",
    label7: "Direction of the bank",
    label8: "Account Number",
    label9: "Name of the beneficiary",
    label10: "Enter the amount to withdraw",

    label11: "Bank name",
    label12: "SWIFT / BIC Code",
    label13: "Direction of the bank",
    label14: "Account Number",
    label15: "SORT Code",
    label16: "IBAN",
    label17: "Enter the amount to withdraw",

    // transferencia_nacional

    label18: "Select payment type",
    option9: "Select a method",
    option10: "Virtual Bank",
    option11: "Physical Bank",

    label18: "Enter your alias or National Account Number (CVU)",
    label19: "Enter the CUIT/CUIL",
    label20: "Enter account name",
    label21: "Enter the amount to withdraw",

    label22: "Enter your alias or National Account Number (CBU)",
    label23: "Enter Savings or Current Account Number",
    label24: "Enter CUIT/CUIL",
    label25: "Enter account name",
    label26: "Enter the amount to withdraw",

    label27: "Enter the amount to withdraw",



    // swal
    messageEmpty: "Please fill in all required fields.",


    swalTitle: "Are you sure about this transaction?",
    swalText: "A withdrawal request will be sent for the amount:",
    swalButton1: "Yes, I'm sure",
    swalButton2: "Cancel",

    swalText2: "to user:",

  },
  pt: {
    title: "Retirar dinheiro",

    label1d1: "Escolha o meio de pagamento",

    option1: "Selecione o método de retirada",
    option2: "Transferência entre usuários",
    option3: "Transferência Externa",
    option4: "Transferências nacionais (Arg)",
    option5: "Efetivo",

    text1:"Você deve verificar sua conta antes de fazer uma retirada. Você pode encontrar informações sobre como fazê-lo em seu perfil.",

    button1: "Enviar",

    // transferencia_entre_usuarios

    label1: "E-mail do destino",
    label2: "Insira o valor a ser sacado",

    // transferencia_nacional

    label3: "Selecione a região",
    option6: "Selecione uma região",
    option7: "USA",
    option8: "Europa",

    label4: "Nome do banco",
    label5: "Routing Number (ACH)",
    label6: "Routing Number (WIRE)",
    label7: "Endereço do banco",
    label8: "Número de conta",
    label9: "Nome do beneficiário",
    label10: "Insira o valor a ser sacado",

    label11: "Nome do banco",
    label12: "Código SWIFT / BIC",
    label13: "Endereço do banco",
    label14: "Número de conta",
    label15: "Código SORT",
    label16: "IBAN",
    label17: "Insira o valor a ser sacado",

    // transferencia_nacional

    label18: "Selecione o tipo de pagamento",
    option9: "Selecione um método",
    option10: "Banco Virtual",
    option11: "Banco Físico",

    label18: "Digite seu alias ou número de conta nacional (CVU)",
    label19: "Insira o CUIT/CUIL",
    label20: "Insira o nome da conta",
    label21: "Insira o valor a ser sacado",

    label22: "Digite seu alias ou número de conta nacional (CBU)",
    label23: " Ingrese el numero de caja de ahorros ó cuenta corriente",
    label24: "Insira o CUIT/CUIL",
    label25: "Insira o nome da conta",
    label26: "Insira o valor a ser sacado",

    label27: "Insira o valor a ser sacado",



    // swal
    messageEmpty: "Por favor preencha todos os campos obrigatórios.",

    swalTitle: "Você está certo desta transação?",
    swalText: "Será enviado um pedido de retirada para a quantidade:",
    swalButton1: "Sim, eu tenho certeza",
    swalButton2: "Cancelar",

    swalText2: "ao usuário:",

  },
};
